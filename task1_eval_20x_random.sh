...
DEFAULT_VELOCITY_HEAD_CHECKPOINT="logs/task1_velocity_head/v12_scaled_stop_decel15/best.pt"
...
MIN_PRE_JUNCTION_ROUTE_POINTS="${MIN_PRE_JUNCTION_ROUTE_POINTS:-8}"
MAX_PRE_JUNCTION_ROUTE_POINTS="${MAX_PRE_JUNCTION_ROUTE_POINTS:-24}"
CACHED_ROUTE_TURN_SPEED_MPS="${CACHED_ROUTE_TURN_SPEED_MPS:-3.0}"
# Use SimLingo speed for every command, including the straight warmup used by
# stop/speed-up/slow-down. The velocity head overrides speed after warmup.
SIMLINGO_SPEED_CONTROL_MODE="${SIMLINGO_SPEED_CONTROL_MODE:-model}"
if [ -z "${VELOCITY_HEAD_CHECKPOINT+x}" ]; then
    if [ ! -f "$DEFAULT_VELOCITY_HEAD_CHECKPOINT" ]; then
        echo "ERROR: required velocity-head checkpoint not found: $DEFAULT_VELOCITY_HEAD_CHECKPOINT" >&2
        echo "Train/copy the checkpoint, set VELOCITY_HEAD_CHECKPOINT to another path, or explicitly set VELOCITY_HEAD_CHECKPOINT=none." >&2
        exit 1
    fi
    VELOCITY_HEAD_CHECKPOINT="$DEFAULT_VELOCITY_HEAD_CHECKPOINT"
elif [ "$VELOCITY_HEAD_CHECKPOINT" != "none" ] && [ ! -f "$VELOCITY_HEAD_CHECKPOINT" ]; then
    echo "ERROR: velocity-head checkpoint not found: $VELOCITY_HEAD_CHECKPOINT" >&2
    exit 1
fi
VELOCITY_HEAD_ARGS=()
...
echo "Min pre-junction route points: $MIN_PRE_JUNCTION_ROUTE_POINTS"
echo "Max pre-junction route points: $MAX_PRE_JUNCTION_ROUTE_POINTS"
echo "Turn speed: ${CACHED_ROUTE_TURN_SPEED_MPS} m/s"
echo "SimLingo speed control mode: $SIMLINGO_SPEED_CONTROL_MODE"
if [ -n "${VELOCITY_HEAD_CHECKPOINT:-}" ] && [ "$VELOCITY_HEAD_CHECKPOINT" != "none" ]; then
...
bash task1_run_by_job.sh "$JOB_ID" run \
    ...
    --duration 20 \
    --eval-duration-overrides "turn left:40,turn right:40,straight:28,stop:22,speed up:28,slow down:28" \
    --fps 10 \
    --sim-fps 20 \
    --policy-hz 0.20 \
    --velocity-head-hz 5.0 \
    --simlingo-speed-control-mode "$SIMLINGO_SPEED_CONTROL_MODE" \
    ...
    --min-pre-junction-route-points "$MIN_PRE_JUNCTION_ROUTE_POINTS" \
    --max-pre-junction-route-points "$MAX_PRE_JUNCTION_ROUTE_POINTS" \
    --cached-route-turn-speed-mps "$CACHED_ROUTE_TURN_SPEED_MPS" \
